...
###########################################
# Create and save Protein-Drug similarity #
###########################################

# Ignore specific pandas PerformanceWarnings
...
dataset_gdsc1['beta'] = abs(dataset_gdsc1['beta'])
dataset_gdsc2['beta'] = abs(dataset_gdsc2['beta'])
print("max1: ", dataset_gdsc1['beta'].max())
print("max2: ",dataset_gdsc2['beta'].max())
dataset_gdsc1['beta'] = dataset_gdsc1['beta']/dataset_gdsc1['beta'].max()
dataset_gdsc2['beta'] = dataset_gdsc2['beta']/dataset_gdsc2['beta'].max()

dataset_gdsc2['y_id'] = dataset_gdsc2['y_id'].str.split(';').str[0]
dataset_gdsc1['y_id'] = dataset_gdsc1['y_id'].str.split(';').str[0]

def create_matrix(dataset3):
    unique_y_ids = dataset3['y_id'].unique()
    unique_x_ids = dataset3['x_id'].unique()

    combined_ids = np.union1d(unique_y_ids, unique_x_ids)
    print(len(unique_y_ids), len(unique_x_ids), len(combined_ids))
    matrix_size = len(combined_ids)
    matrix = pd.DataFrame(np.zeros((matrix_size, matrix_size)), index=combined_ids, columns=combined_ids)
    counter100 = 0
    for index, row in dataset3.iterrows():
        y_id = row['y_id']
        x_id = row['x_id']
        beta_value = float(row['beta'])
        #if not y_id in matrix.index or not x_id in matrix.columns:
        #    print(y_id, x_id)
        matrix.at[y_id, x_id] = beta_value
        matrix.at[x_id, y_id] = beta_value
        counter100 += 1
        print("counter100", counter100)
    return matrix, combined_ids

matrix_gdsc1, node_names_gdsc1 = create_matrix(dataset_gdsc1)
matrix_gdsc2, node_names_gdsc2 = create_matrix(dataset_gdsc2)
...
similarity_matrix_gdsc1 = matrix_gdsc1.values
similarity_matrix_gdsc2 = matrix_gdsc2.values
# Convert the NumPy array to a DataFrame
df_similarity_matrix_gdsc1 = matrix_gdsc1
df_similarity_matrix_gdsc2 = matrix_gdsc2
# Save the DataFrame to a CSV file
df_similarity_matrix_gdsc1.to_csv('protein_drug/similarity_matrix_gdsc1.csv')
df_similarity_matrix_gdsc2.to_csv('protein_drug/similarity_matrix_gdsc2.csv')
...
########################
print("start")
tsne_result_gdsc1_30 = TSNE(n_components=2, random_state=0, n_iter=1000, n_iter_without_progress=300, perplexity=30).fit_transform(similarity_matrix_gdsc1)
tsne_result_gdsc2_30 = TSNE(n_components=2, random_state=0, n_iter=1000, n_iter_without_progress=300, perplexity=30).fit_transform(similarity_matrix_gdsc2)
print("stop")
tsne_result_gdsc1_5 = TSNE(n_components=2, random_state=0, n_iter=1000, n_iter_without_progress=300, perplexity=5).fit_transform(similarity_matrix_gdsc1)
tsne_result_gdsc2_5 = TSNE(n_components=2, random_state=0, n_iter=1000, n_iter_without_progress=300, perplexity=5).fit_transform(similarity_matrix_gdsc2)

isomap_result_gdsc1_30 = Isomap(n_components=2, n_neighbors=30).fit_transform(similarity_matrix_gdsc1)
isomap_result_gdsc2_30 = Isomap(n_components=2, n_neighbors=30).fit_transform(similarity_matrix_gdsc2)
isomap_result_gdsc1_5 = Isomap(n_components=2, n_neighbors=5).fit_transform(similarity_matrix_gdsc1)
isomap_result_gdsc2_5 = Isomap(n_components=2, n_neighbors=5).fit_transform(similarity_matrix_gdsc2)

#umap_result_gdsc1_5 = umap.UMAP(n_components=2, random_state=0, n_neighbors=5, min_dist=0.1).fit_transform(similarity_matrix_gdsc1)
#umap_result_gdsc2_5 = umap.UMAP(n_components=2, random_state=0, n_neighbors=5, min_dist=0.1).fit_transform(similarity_matrix_gdsc2)
#umap_result_gdsc1_30 = umap.UMAP(n_components=2, random_state=0, n_neighbors=30, min_dist=0.1).fit_transform(similarity_matrix_gdsc1)
#umap_result_gdsc2_30 = umap.UMAP(n_components=2, random_state=0, n_neighbors=30, min_dist=0.1).fit_transform(similarity_matrix_gdsc2)
umap_result_gdsc1 = umap.UMAP(n_components=2, random_state=0, n_neighbors=15, min_dist=0.1).fit_transform(similarity_matrix_gdsc1)
umap_result_gdsc2 = umap.UMAP(n_components=2, random_state=0, n_neighbors=15, min_dist=0.1).fit_transform(similarity_matrix_gdsc2)
communities1_bp = community_louvain.best_partition(G1)
communities2_bp = community_louvain.best_partition(G2)
...
num_communities1_2 = max(communities1_bp.values()) + 1
community_centers1_2 = calculate_community_centers(num_communities1_2)
community_node_positions1_2 = calculate_community_positions(community_centers1_2, num_communities1_2, communities1_bp)


num_communities2_2= max(communities2_bp.values()) + 1
community_centers2_2 = calculate_community_centers(num_communities2_2)
community_node_positions2_2 = calculate_community_positions(community_centers2_2, num_communities2_2, communities2_bp)
...
def writeDB(tx, node_names, tsne_x, tsne_y, gdsc, methode):
    ...
    for node_name, x, y in zip(node_names, tsne_x, tsne_y):
        ...
        if node_name.isdigit():
            ...
            SET n.drug_protein_{gdsc}_{methode} = 1
            SET n.drug_protein_{gdsc}_{methode}_x = {x}
            SET n.drug_protein_{gdsc}_{methode}_y = {y}
            """
        else:
            drug_id = node_name
            query = f"""
            MATCH (n:Protein {{name: "{drug_id}", project_id: 1}})
            SET n.drug_protein_{gdsc}_{methode} = 1
            SET n.drug_protein_{gdsc}_{methode}_x = {x}
            SET n.drug_protein_{gdsc}_{methode}_y = {y}
            """
...
def writeDBCom(tx, com, gdsc, methode):
    ...
    for key, value in com.items():
        ...
        if node_name.isdigit():
            ...
            query = f"""
            MATCH (n:Drug {{drug_id: "{drug_id}"}})
            SET n.drug_protein_{gdsc}_{methode} = {value}
            """
        else:
            drug_id = node_name
            query = f"""
...
"""
updateDB2(node_names_gdsc1, tsne_result_gdsc1_30[:, 0], tsne_result_gdsc1_30[:, 1], "GDSC1", "tsne_global")
updateDB2(node_names_gdsc2, tsne_result_gdsc2_30[:, 0], tsne_result_gdsc2_30[:, 1], "GDSC2", "tsne_global")
updateDB2(node_names_gdsc1, tsne_result_gdsc1_5[:, 0], tsne_result_gdsc1_5[:, 1], "GDSC1", "tsne_local")
updateDB2(node_names_gdsc2, tsne_result_gdsc2_5[:, 0], tsne_result_gdsc2_5[:, 1], "GDSC2", "tsne_local")

updateDB2(node_names_gdsc1, isomap_result_gdsc1_30[:, 0]*5, isomap_result_gdsc1_30[:, 1]*5, "GDSC1", "isomap_global")
updateDB2(node_names_gdsc2, isomap_result_gdsc2_30[:, 0]*5, isomap_result_gdsc2_30[:, 1]*5, "GDSC2", "isomap_global")
updateDB2(node_names_gdsc1, isomap_result_gdsc1_5[:, 0]*5, isomap_result_gdsc1_5[:, 1]*5, "GDSC1", "isomap_local")
updateDB2(node_names_gdsc2, isomap_result_gdsc2_5[:, 0]*5, isomap_result_gdsc2_5[:, 1]*5, "GDSC2", "isomap_local")



updateDB2(node_names_gdsc1, umap_result_gdsc1_30[:,0]*5, umap_result_gdsc1_30[:,1]*5, "GDSC1", "umap_global")
updateDB2(node_names_gdsc2, umap_result_gdsc2_30[:,0]*5, umap_result_gdsc2_30[:,1]*5, "GDSC2", "umap_global")
updateDB2(node_names_gdsc1, umap_result_gdsc1_5[:,0]*5, umap_result_gdsc1_5[:,1]*5, "GDSC1", "umap_local")
updateDB2(node_names_gdsc2, umap_result_gdsc2_5[:,0]*5, umap_result_gdsc2_5[:,1]*5, "GDSC2", "umap_local")
"""
updateDB2(node_names_gdsc1, umap_result_gdsc1[:,0]*5, umap_result_gdsc1[:,1]*5, "GDSC1", "umap")
updateDB2(node_names_gdsc2, umap_result_gdsc2[:,0]*5, umap_result_gdsc2[:,1]*5, "GDSC2", "umap")
"""
updateDB2(community_node_positions1[:,0], community_node_positions1[:,1], community_node_positions1[:,2], "GDSC1", "louvain_global")
updateDB2(community_node_positions2[:,0], community_node_positions2[:,1], community_node_positions2[:,2], "GDSC2", "louvain_global")
updateDB2(community_node_positions1_2[:,0], community_node_positions1_2[:,1], community_node_positions1_2[:,2], "GDSC1", "louvain_local")
updateDB2(community_node_positions2_2[:,0], community_node_positions2_2[:,1], community_node_positions2_2[:,2], "GDSC2", "louvain_local")
updateDBCommunity("GDSC1",  "louvain_global", communities1)
updateDBCommunity("GDSC2",  "louvain_global", communities2)
updateDBCommunity("GDSC1",  "louvain_local", communities1_bp)
updateDBCommunity("GDSC2",  "louvain_local", communities2_bp)
"""
