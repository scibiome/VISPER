...
def query_create_connections(tx):
    query = """
            ...
            SET d.drug_owner = row.drug_owner,
                d.webrelease = row.webrelease,
                d.drug_id = row.drug_id,
                d.model_type = row.model_type,
                d.PUBCHEM = row.PUBCHEM,
                d.target_pathway = row.target_pathway,
                d.putative_gene_target = row.putative_gene_target,
                d.model_treatment = row.model_treatment,
                d.drug_synonyms = row.drug_synonyms,
                d.putative_target = row.putative_target,
                d.CHEMBL = row.CHEMBL,
                d.use_in_publications = row.use_in_publications
