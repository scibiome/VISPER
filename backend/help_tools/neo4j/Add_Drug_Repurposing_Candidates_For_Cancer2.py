...
def extract_drugbank_values(tx, file_path):
    ...
    chembl_ids_query = """
    MATCH (d:Drug)
    RETURN d.uid AS uid, d.DrugBank AS DrugBank, d.name AS name
    """
    chembl_ids_result = tx.run(chembl_ids_query)
    chembl_ids = [[record["uid"],record["DrugBank"], record["name"]]  for record in chembl_ids_result]
    count = 0
    with open(file_path, 'r') as file:
        headers = file.readline().strip().split('\t')

        try:
            drugbank_index = headers.index('DrugBank')
            name = headers.index('Drug_INN')
        except ValueError:
            return []
        counter = 0
        for line in file:
            row = line.strip().split('\t')
            if len(row) > drugbank_index:  
                for c1 in chembl_ids:
                    if  (row[drugbank_index] == c1[1] and row[drugbank_index] != "") or  row[name] == c1[2]:
                        chembl_ids_query2 = """
                        MATCH (d:Drug)
                        WHERE d.uid = $uid
                        SET d.Candidate = 1
                        """
                        tx.run(chembl_ids_query2, uid=c1[0])
                        count = count + 1
                        print(count)
                        break


    return drugbank_values

def update_database_with_drugbank():   
    with GraphDatabase.driver(uri, auth=(username, password)) as driver:
        with driver.session() as session:
            session.write_transaction(extract_drugbank_values, file_path='info.txt')
